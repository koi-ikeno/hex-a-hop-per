...
public:
	...
	LevelSave* GetLevel(std::string name, bool create)
	{
		const char * l = strstr(name.c_str(), "Levels");
		if (l)
			name = l;

		X* x = first;
		if (x && strcmp(name.c_str(), x->name)==0) return x;
		while (x && x->next)
		{
			if (strcmp(name.c_str(), x->next->name)==0) return x->next;
			x = x->next;
		}
		if (create)
		{
			X* n = new X(name.c_str());
			if (x)
				x->next = n;
			else
				first = n;
