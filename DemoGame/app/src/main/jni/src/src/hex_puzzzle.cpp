...
extern "C" jint Java_koisignal_hexahop_per_SDLSurface_getCurrentStageJHH( JNIEnv* env,
																   jobject thiz )
{
...
*/
	...
	#define FONT_SPACING 26 //for spacing between lines
...
struct HexPuzzle : public State
{
	...
	LevelInfo* GetLevelInfo(const char* f)
	{
		if (strstr(f, "Levels\\") == f)
			f += 7;
		if (currentLevelInfo!=0 && strcmp(currentLevelInfo->file.c_str(), f)==0)
		...
		for (unsigned int i=0; i<sizeof(levelNames)/sizeof(levelNames[0]); i++)
			if (strcmp(f, levelNames[i].file.c_str())==0) {
				currentGameStage = i;
				return &levelNames[i];
			}
...
//		time = 1;	// Guarantee we can't try and do things at time=0
	...
	*/
	...
	int GetPar(const char * level, bool getdiff=false)
	{
		...
		#ifdef USE_LEVEL_PACKFILE
			int Id = levelFiles.SetLevel(level);
			if (Id == -1) return 999;
			//loadPtr = (char*)e->Data();
	...
	bool LoadSave(std::string filename, bool save)
	{
		if (filename.size() == 0) 
			return false;
		...
		if (!save)
		{
			...
			if (!save)
			{
				int Id = levelFiles.SetLevel(filename);
				//������ID�ɂ���
   ...
   */
	void _LoadSave(FILE* f, bool save)
	{
		...
		if (version<=4)
		{
			...
			bounds[0] = levelFiles.GetLevelDataVec()[11];
			bounds[1] = levelFiles.GetLevelDataVec()[12];
			bounds[2] = levelFiles.GetLevelDataVec()[13];
			bounds[3] = levelFiles.GetLevelDataVec()[14];
...
//					offsety = 2 - bounds[2];
			...
			unsigned int itr_data = 23;
			for (int i = bounds[0]; i <= bounds[1]; i++) {
				for (int j = bounds[2]; j <= bounds[3]; j++)
				{
					unsigned char comp = map[i][j] | (map_item[i][j] << 5);
					//fn(&comp, sizeof(comp), 1, f);
					comp = levelFiles.GetLevelDataVec()[itr_data];
					itr_data++;
					map[i][j] = comp & 0x1f;
			...
			else
			{
				while(1){
					int16_t len;
					unsigned char x, y;
					x = levelFiles.GetLevelDataVec()[itr_data];
					itr_data++;
					y = levelFiles.GetLevelDataVec()[itr_data];
					itr_data++;
					x += offsetx; y += offsety;
					//fn(&len, sizeof(len), 1, f);
					...
					for (auto i = 0 ; i < len; i++) {
						tmp[i] = levelFiles.GetLevelDataVec()[itr_data];
						itr_data++;
					}
...
//			if (i>1 || i==1 && t>1)
	...
	SDL_Surface* Load(const char * bmp, bool colourKey=true)
	{
		...
		SDL_Surface* out = SDL_ConvertSurfaceFormat(g, 
	...
	//			SDL_Rect src = {0, 0, uiGraphics->w, uiGraphics->h};
		...
		//		dst.x += src.w/2;
				...
				if (currentLevelInfo)
				{
					...
					PrintC(true, dst.x, dst.y - FONT_SPACING/4, currentLevelInfo->name.c_str());
