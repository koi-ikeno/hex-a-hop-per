...
#define MAX_GAMESLOT 4 //set 4. or menu options will cllapse.
#include <string>
...
struct Menu {
	...
	Menu() : renderBG(true), under(activeMenu), time(0) { 
		activeMenu = this; 
	}
...
/* TRANSLATORS: This string is copied twice into the POT file to workaround a
   ...
   will not be used. */
_("Welcome to  hex-a-hopper!    This is a puzzle game based on hexagonal tiles.  There is no time limit and no real-time element, so  take as long as you like.    Use the cursor keys or click on the arrows to  scroll through the help pages. More pages will be  added as you progress through the game.")
};
...
{
	sprintf(t, "/data/user/0/koisignal.hexahop.per/files/save%d.dat", i+1);
...
//				opt[num_opt++] = OPT_GAMESLOT_0 + i;
		...
		r.y -= FONT_SPACING *2.5,
		r2.y -= FONT_SPACING *2.5;
		r.x -= FONT_SPACING * 1.5;

		if (num_opt==3){
			r.y+=FONT_SPACING+FONT_SPACING/2,
			r2.y+=FONT_SPACING+FONT_SPACING/2;
		}
